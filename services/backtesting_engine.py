...
class BacktestingEngine:
    ...
    def _generate_signal(self, weather: HistoricalWeatherPoint,
        ...
        # Dynamic consensus threshold based on signal strength
        abs_delta = abs(weather.delta)
        if abs_delta > 4.0:  # Strong signal
            consensus_threshold = 0.65  # Lower threshold for strong signals
        elif abs_delta > 2.5:  # Moderate signal
            consensus_threshold = 0.70  # Standard threshold
        else:  # Weak signal
            consensus_threshold = 0.75  # Higher threshold for weak signals
        
        if ensemble_consensus < consensus_threshold:
